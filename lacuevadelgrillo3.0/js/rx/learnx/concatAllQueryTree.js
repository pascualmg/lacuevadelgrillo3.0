...
function predicateFunctionLosMenoresDeCinco(item)
{
   return item < 5;
};

function projectionFunctionToHumanWord(item)
{
    var numberName = [
        'cero',
        'uno',
...
console.log('esto lo podemos filtrar', treeFoo.miConcatAll().miFilter(predicateFunctionLosMenoresDeCinco));
console.log('y luego lo podemos mapear o lo que haga falta...',
    treeFoo
        .miConcatAll()
        .miFilter(predicateFunctionLosMenoresDeCinco)
        .miMap(projectionFunctionToHumanWord)
);

//funcion del ejercicio 9, con un foreach
function concatAllQyeryTree() {
    var movieLists = [
            {
                name: "New Releases",
                videos: [
